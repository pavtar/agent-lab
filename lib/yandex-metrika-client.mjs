...
export function getDateRange(days = 30) {
  const fromEnv1 = process.env.METRIKA_REPORT_DATE1?.trim();
  const fromEnv2 = process.env.METRIKA_REPORT_DATE2?.trim();
  if (fromEnv1 && fromEnv2) {
    return { date1: fromEnv1, date2: fromEnv2 };
  }
